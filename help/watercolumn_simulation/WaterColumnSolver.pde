// Taken and partially ported from: https://github.com/d3p/algorithmic-cliches
class WaterColumnSolver {
    public float fluidDensity = 0.1f; /* the density of the fluid */
    public float gravitation = 9.81f; /* gravitational force */
    public float pipeLength = 1.0f; /* length of a connection pipe */
    public float pipeArea = 0.0007f * 10; /* the cross-sectional area of a pipe */
    ...
    public WaterColumnSolver(int pGridX, int pGridY) {
     ...
     */
    public WaterColumnSolver(int pGridX, int pGridY, float pInitialVolume) {
    ...
    private void computePipeFlows(final float pDeltaTime) {
        final float p_cacc = (pDeltaTime * pipeArea * gravitation) / (pipeLength * (1.0f / (mGridY * mGridX)));
    ...
    public void applyForce(int x, int y, final float pMagnitude, final float pDeltaTime) {
        final float p_cforce = (pDeltaTime * pipeArea) / (pipeLength * fluidDensity);
