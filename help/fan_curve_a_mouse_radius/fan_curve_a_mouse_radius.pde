...
void draw() {
  ...
  float sig = map(mouseX, 0, width, 0.2, 1.0);
  ...
  float x1 = center.x + cos(angle) * rad * sig;
  float y1 = center.y + sin(angle-rad) * rad * sig;
  float z1 = center.z + sin(angle) * rad * sig;

  float x2 = center.x - cos(angle) * rad2 * sig;
  float y2 = center.y - sin(angle-rad2) * rad2 * sig;
  float z2 = center.z - sin(angle) * rad2 * sig;
